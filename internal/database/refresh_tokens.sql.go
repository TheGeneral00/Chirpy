...
type RetrieveRefreshTokenRow struct {
	Token     string
	ExpiresAt time.Time
...
func (q *Queries) RetrieveRefreshToken(ctx context.Context, token string) (RetrieveRefreshTokenRow, error) {
	row := q.db.QueryRowContext(ctx, retrieveRefreshToken, token)
	var i RetrieveRefreshTokenRow
	err := row.Scan(
		&i.Token,
		&i.ExpiresAt,
		&i.RevokedAt,
