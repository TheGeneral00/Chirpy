...
const deleteChrip = `-- name: DeleteChrip :one
DELETE FROM chirps WHERE id = $1
RETURNING id
`

func (q *Queries) DeleteChrip(ctx context.Context, id uuid.UUID) (uuid.UUID, error) {
	row := q.db.QueryRowContext(ctx, deleteChrip, id)
	err := row.Scan(&id)
	return id, err
}
